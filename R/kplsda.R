kplsda <- function(Xr, Yr, Xu, Yu = NULL, ncomp, da = dalm, 
  ...
  for(i in 1:nrow(kpar)) {
    ...
    zfm <- switch(namkern,
      ...
      kpol = .kplsda(Xr, Yr, Xu, Yu, ncomp, da, kern = kpol, weights, 
                 degree = kpar[i, "degree"], scale = kpar[i, "scale"], offset = kpar[i, "offset"]),
      
      krbf = .kplsda(Xr, Yr, Xu, Yu, ncomp, da, kern = krbf, weights, 
                 sigma = kpar[i, "sigma"]),

      ktanh = .kplsda(Xr, Yr, Xu, Yu, ncomp, da, kern = ktanh, weights, 
